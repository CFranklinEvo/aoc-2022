...
function calculateString($calc) {
    ...
    switch ($terms[1]) {
        case '+':
            $result = gmp_add($terms[0], $terms[2]);
            break;
        case '-':
            $result = gmp_sub($terms[0], $terms[2]);
            break;
        case '/':
            $result = gmp_div($terms[0], $terms[2]);
            break;
        case '*':
            $result = gmp_mul($terms[0], $terms[2]);
            break;
        default:
...
for ($round = 1; $round <= $rounds; $round++) {
    foreach ($monkeys as $monkeyId => $monkeh) {
        ...
        foreach ($items as $item) {
            ...
            $worry = $divide ? floor(gmp_div($worry, 3)) : $worry;

            if (gmp_mod($worry, $monkey['test']) === 0) {
...
print_r("Part 1: Monkey Business = " . (gmp_mul($counts[0], $counts[1])) . PHP_EOL);
